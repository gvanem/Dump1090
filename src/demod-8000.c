    ...
    if (window || (max * 2 > phase_av * 3))
    {
      ...
      if (++window == D8M_WIN_LEN)
      {
        ...
        for (msg_type = 0; msg_type < 2; msg_type++)
        {
          ...
          {
            message_result = mode_message_score (msg, msg_bytes * 8);
        ...
        if (best_result >= 0)
        {
          ...
          message_result = mode_message_decode (&mm, best_msg);
