...
def main():
    ...
    font_cache = {
        ...
        "bottom": utils.load_font (config.BOTTOM_FONT_SIZE)
      }
    ...
    while running:
        ...
        #
        # Bottom-text with clickable areas (centered under radar scope)
        #
        quit_text = "Q/ESC: QUIT"
        if audio and audio.initialised:
           audio_text = "A: ATC " + ["[OFF]", "[ON]" ] [audio.is_playing()]
        else:
           audio_text = ""
        ...
        #
        bottom_text = quit_text
        if audio_text:
           bottom_text += "    " + audio_text

        bottom_surface = font_cache ["bottom"].render (bottom_text, True, config.DIM_GREEN)

        #
        # Centre the bottom-text under the radar scope (same centerx as radar title)
        #
        bottom_rect = bottom_surface.get_rect (centerx = config.SCREEN_WIDTH // 4, y = config.SCREEN_HEIGHT - 55)

        #
        # For hover/click, calculate the rects for each part
        #
        quit_surface = font_cache ["bottom"].render (quit_text, True, config.DIM_GREEN)
        ...
        #
        quit_rect.x = bottom_rect.x

        if audio_text:
           audio_surface = font_cache ["bottom"].render (audio_text, True, config.DIM_GREEN)
           ...
           #
           audio_rect.x = quit_rect.right + font_cache ["bottom"].size("    ")[0]
        else:
           audio_surface = None
           audio_rect = None

        #
        # Mouse-hover effects for bottom-text
        #
        ...
        #
        quit_surface = font_cache ["bottom"].render (quit_text, True, quit_col)
        screen.blit (quit_surface, quit_rect)
        if audio_surface and audio_rect:
           audio_surface = font_cache ["bottom"].render (audio_text, True, audio_col)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT or (event.type == pygame.KEYDOWN and event.key in (pygame.K_q, pygame.K_ESCAPE)):
               ...
               #
               # Check for clicks on bottom text areas
               #
